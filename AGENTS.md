...
| 9 UI | ✅ | HUD overlay (`src/ui/HUD.tsx`): jarak ke gawang live (DistanceMeter via interval+DOM, no re-render), badge AREA TENDANG, tombol Restart. Responsive (clamp/flex). |
| 10 Polish | ✅ | **Game lengkap.** Audio (footstep loop + kick/goal one-shot, `AudioSystem.ts`), mute toggle di HUD. Kamera cinematic: damping + camera-collision (Rapier raycast, grup `cameraRay`). |
...
- 2026-06-20 — Fase 10 selesai → **game lengkap (semua fase ✅)**. (a) Audio WAV mentah (footstep 23s/3.9MB, dll) dikompres ffmpeg → MP3 mono (footstep 64k/182KB, kick 96k/17KB, goal 112k/73KB); mentah di `assets-raw/audio/` (gitignore). (b) `AudioSystem.ts` (HTMLAudio): `footstep` loop di-gate locomotion (rate 1.0 walk / 1.5 run, pause saat Idle/win), `kick`/`goal` one-shot (clone node). Semua `play()` di-catch (autoplay policy; bunyi setelah gesture WASD pertama). Trigger: footstep di Player, kick di Ball saat impulse (sinkron kontak), goal di GoalNet. Mute toggle di HUD. (c) CameraRig: tambah camera-collision via Rapier `world.castRay` (rapier 0.14 → `hit.timeOfImpact`), grup `cameraRay` (membership PLAYER, filter WORLD) supaya ray hanya kena gedung/gawang/ground; kamera didekatkan (MIN_DIST 1.5, margin 0.3) bila ketutup tembok.
- 2026-06-20 — Fase 9 selesai. HUD = overlay DOM (App, di luar Canvas). `DistanceMeter` hitung jarak `GOAL_LINE_Z - playerPosition.z` via `setInterval(100ms)` + tulis `textContent` langsung (hindari re-render React tiap frame). HUD reaktif hanya ke `inGoalZone`/`gameState`. Tombol Restart panggil `reset()`. Styling responsive pakai `clamp()`; container `pointer-events:none`, tombol `auto`.
